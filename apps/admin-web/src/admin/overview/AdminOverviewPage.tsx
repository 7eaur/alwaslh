...
export function AdminOverviewPage({ onSessionExpired }: { onSessionExpired: () => void }) {
  ...
  return (
    <section className="admin-overview-page">
      ...
      {state === "loading" ? (
        <AdminProductState title="جارٍ تحميل العمل المعلّق" body="نقرأ الحالة الحالية من الخادم." />
      ) : state === "error" || !summary ? (
        <AdminProductState title="تعذر تحميل النظرة العامة" body={message} onRetry={() => void load()} />
      ) : (
        <>
          {attention.length === 0 ? (
