...
export function QuizBuilderWorkspace({ onSessionExpired }: Props) {
  ...
  async function submitCreate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({ kind: "success", text: "تم إنشاء الاختبار كمسودة. سيتم فتح صفحة الاختبار لإدارة النماذج." });
  ...
  return (
    <section className="quiz-builder" aria-labelledby="quiz-create-heading">
      <header className="workspace-header">
        <div>
          ...
          <p className="workspace-lede">
            أنشئ مسودة الاختبار وحدد نطاقها التعليمي. بعد الإنشاء تنتقل إدارة النماذج والمراجعة والنشر إلى صفحة
            الاختبار نفسها.
          </p>
        </div>
      </header>
      ...
      {feedback ? (
        <p className={`inline-feedback ${feedback.kind}`} role="status">
          {feedback.text}
        </p>
      ) : null}
