...
export function App() {
  ...
  const restore = useCallback(async () => {
    ...
    try {
      ...
      setSession(null);
      if (isMissingSessionError(error)) { setSessionState("signed_out"); return; }
  ...
  return <AdminShell profile={session} onSessionExpired={() => { setSession(null); setSessionState("signed_out"); }} onLogout={async () => { try { await logoutAdmin(); setSession(null); setSessionState("signed_out"); } catch (error) { setSessionError(errorMessage(error)); setSessionState("error"); } }} />;
}
    ...
    <div className="admin-shell">
      <aside className="admin-sidebar" aria-label="التنقل الرئيسي">
        ...
        <nav className="admin-nav" aria-label="أقسام الإدارة">
          {ADMIN_NAVIGATION.map((group) => <section className="admin-nav-group" key={group.label} aria-label={group.label}><p className="admin-nav-label">{group.label}</p><div className="admin-nav-links">{group.items.map((item) => <NavLink className={({ isActive }) => `nav-item${isActive ? " is-active" : ""}`} key={item.to} to={item.to} end={item.end}>{item.label}</NavLink>)}</div></section>)}
        </nav>
      ...
      <main className="admin-main"><AdminRoutes onSessionExpired={onSessionExpired} /></main>
    </div>
  );
}
...
function AdminRoutes({ onSessionExpired }: { onSessionExpired: () => void }) {
  return (
    <Routes>
      ...
      <Route path="content" element={<WorkspaceWithRelatedActions actions={[{ label: "أدوات التأليف بالذكاء الاصطناعي", to: "/app/tools/ai-authoring" }]}><ContentIngestionWorkspace onSessionExpired={onSessionExpired} /></WorkspaceWithRelatedActions>} />
      <Route path="reviews" element={<Navigate replace to="/app/reviews/content" />} />
      <Route path="reviews/content" element={<ReviewArea current="content"><ContentOperationsWorkspace onSessionExpired={onSessionExpired} /></ReviewArea>} />
      <Route path="reviews/ai" element={<ReviewArea current="ai"><AiOperationsPage onSessionExpired={onSessionExpired} /></ReviewArea>} />
      <Route path="questions" element={<WorkspaceWithRelatedActions actions={[{ label: "أدوات التأليف بالذكاء الاصطناعي", to: "/app/tools/ai-authoring" }]}><QuestionBankWorkspace onSessionExpired={onSessionExpired} /></WorkspaceWithRelatedActions>} />
      <Route path="quizzes" element={<WorkspaceWithRelatedActions actions={[{ label: "أدوات التأليف بالذكاء الاصطناعي", to: "/app/tools/ai-authoring" }]}><QuizBuilderWorkspace onSessionExpired={onSessionExpired} /></WorkspaceWithRelatedActions>} />
      <Route path="students" element={<AdminStudentAccessWorkspace onSessionExpired={onSessionExpired} />} />
      <Route path="access-codes" element={<WorkspaceWithRelatedActions actions={[{ label: "الملفات والتقارير", to: "/app/access-codes/reports" }]}><AdminStudentAccessWorkspace onSessionExpired={onSessionExpired} /></WorkspaceWithRelatedActions>} />
...
function ReviewArea({ current, children }: { current: "content" | "ai"; children: ReactNode }) {
  return <><nav className="workspace-subnav" aria-label="أنواع المراجعة"><NavLink className={`workspace-subnav-link${current === "content" ? " is-active" : ""}`} to="/app/reviews/content" aria-current={current === "content" ? "page" : undefined}>المحتوى وOCR</NavLink><NavLink className={`workspace-subnav-link${current === "ai" ? " is-active" : ""}`} to="/app/reviews/ai" aria-current={current === "ai" ? "page" : undefined}>مخرجات الذكاء الاصطناعي</NavLink></nav>{children}</>;
}

function WorkspaceWithRelatedActions({ actions, children }: { actions: readonly { label: string; to: string }[]; children: ReactNode }) {
  return <><nav className="workspace-related-actions" aria-label="إجراءات مرتبطة">{actions.map((action) => <Link className="secondary-button workspace-related-link" key={action.to} to={action.to}>{action.label}</Link>)}</nav>{children}</>;
}

function AdminRouteNotFound() {
  return <section className="auth-card admin-route-state"><p className="eyebrow">مساحة الإدارة</p><h1>هذه الوجهة غير موجودة</h1><p>استخدم التنقل الرئيسي للوصول إلى المهمة المطلوبة.</p><Link className="primary-button admin-route-state-link" to="/app">العودة إلى النظرة العامة</Link></section>;
}
