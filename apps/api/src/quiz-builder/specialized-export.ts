...
import type { QuizBuilderDetail, QuizBuilderQuestionView, QuizBuilderService, QuizBuilderVersionView } from "./service.js";
...
function selectedVersions(detail: QuizBuilderDetail, versionIds: readonly string[]): QuizBuilderVersionView[] {
  ...
  if (ids.length === 0 || ids.length > 20) throw new AppError("BAD_REQUEST", "اختر من نموذج واحد إلى 20 نموذجًا", 400);
  const versionMap = new Map(detail.versions.map((version) => [version.id, version]));
  const versions = ids.map((id) => versionMap.get(id));
  if (versions.some((version) => !version)) throw new AppError("NOT_FOUND", "أحد النماذج المحددة غير موجود", 404);
...
function questionHtml(question: QuizBuilderQuestionView, index: number, variant: QuizPrintVariant): string {
  const options = question.options.length
    ? `<ol class="options">${question.options.map((option) => `<li>${htmlEscape(option.text)}</li>`).join("")}</ol>`
    : "";
  const answer = `<section class="answer"><strong>الإجابة:</strong> ${htmlEscape(correctAnswer(question))}</section>`;
  const explanation = question.explanation
    ? `<section class="explanation"><strong>الشرح:</strong> ${htmlEscape(question.explanation)}</section>`
    : "";
  if (variant === "answer_key") return `<p>${index + 1}. ${htmlEscape(correctAnswer(question))}</p>`;
  if (variant === "answers_explanations") return `<article class="question"><h2>${index + 1}. ${htmlEscape(question.prompt)}</h2>${answer}${explanation}</article>`;
  const includeOptions = variant === "questions_options" || variant === "questions_answers";
  const includeAnswer = variant === "questions_answers";
  return `<article class="question"><h2>${index + 1}. ${htmlEscape(question.prompt)}</h2><p class="meta">${htmlEscape(typeLabel(question.type))}</p>${includeOptions ? options : ""}${includeAnswer ? answer : ""}</article>`;
}
...
export class QuizSpecializedExportService {
  ...
  async bundle(quizId: string, versionIds: readonly string[], variant: QuizPrintVariant): Promise<QuizSpecializedExportBundle> {
    const detail = await this.quizzes.detail(quizId);
    const versions = selectedVersions(detail, versionIds);
    const filenameBase = safeFilename(`${detail.quiz.title}-${versions.map((version) => version.label).join("-")}`);
    let body = "";
    if (variant === "lesson_names") {
      body = `<section><h2>دروس الاختبار</h2><ol class="lesson-list">${detail.lessons.map((lesson) => `<li>${htmlEscape(lesson.title)}</li>`).join("")}</ol></section>`;
    } else if (variant === "lesson_images") {
      const assets = await this.lessonImageAssets(quizId);
      body = assets
        .map(
          (asset) => `<figure class="lesson-image"><figcaption>${htmlEscape(asset.lesson_title)} · صفحة ${asset.position + 1}</figcaption><img src="/v1/admin/quizzes/${encodeURIComponent(quizId)}/export-assets/${encodeURIComponent(asset.asset_id)}" alt="${htmlEscape(asset.lesson_title)}" /></figure>`,
        )
        .join("");
      if (!body) throw new AppError("CONFLICT", "لا توجد صور دروس منشورة قابلة للتصدير", 409);
    } else {
      body = versions
        .map(
          (version) => `<section class="version"><h2>${htmlEscape(version.label)}</h2>${version.questions.map((question, index) => questionHtml(question, index, variant)).join("")}</section>`,
        )
        .join("");
    }
    return { filenameBase, csv: buildCsv(detail.quiz.title, versions), printHtml: baseHtml(detail.quiz.title, body) };
  }

  async asset(quizId: string, assetId: string): Promise<{ mimeType: string; bytes: Buffer }> {
    const rows = await this.database.query<{ storage_key: string; mime_type: string }>(
    ...
    const asset = rows[0];
    if (!asset) throw new AppError("NOT_FOUND", "صورة الدرس غير متاحة لهذا الاختبار", 404);
    return { mimeType: asset.mime_type, bytes: await this.storage.read(asset.storage_key) };
  }

  private lessonImageAssets(quizId: string) {
    return this.database.query<{ asset_id: string; lesson_title: string; position: number }>(
      `select la.id as asset_id, l.title as lesson_title, la.position
       ...
       where ql.quiz_id = $1 and la.media_asset_id is not null
         and exists (select 1 from media_variants v where v.media_asset_id = la.media_asset_id and v.kind = 'display')
