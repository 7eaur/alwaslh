...
): Pick<AiGenerationRequest, "language" | "subjectDomain" | "sourceSensitivity" | "notationPolicy" | "sourceChunks"> {
...
function questionRequest(
  ...
  subjectDomain: AiSubjectDomain,
  sourceChunks: AiSourceChunk[],
  options: { lessonTitle?: string; target?: AiQuestionTarget; expectedQuestionCount?: number },
): AiGenerationRequest {
  const base = baseRequest(mode, subjectDomain, sourceChunks);
  if (mode === "lesson_summary") {
    return aiGenerationRequestSchema.parse({ ...base, mode, lessonTitle: options.lessonTitle });
  }
  if (mode === "question_generation") {
    if (!options.target) throw new AppError("BAD_REQUEST", "أعداد أنواع الأسئلة مطلوبة", 400);
...
export class AdminAiAuthoringService {
  ...
  async enqueueLessons(actorProfileId: string, input: LessonGenerationInput): Promise<AuthoringPlanResult> {
    if (!LESSON_MODES.has(input.mode)) throw new AppError("BAD_REQUEST", "وضع توليد الدروس غير مدعوم", 400);
    ...
    const units = lessonIds.map((lessonId) => {
      ...
      return {
        ...
        request: questionRequest(input.mode, input.subjectDomain, sourceChunks, {
    ...
    return planView(
      await this.createPlan(actorProfileId, `admin-authoring:${input.clientRequestId}`, input.priority ?? 5, units, authoring),
    );
  }

  async enqueueQuiz(actorProfileId: string, quizId: string, input: QuizGenerationInput): Promise<AuthoringPlanResult> {
    if (!QUIZ_MODES.has(input.mode)) throw new AppError("BAD_REQUEST", "وضع توليد الاختبار غير مدعوم", 400);
    if (input.versions.length === 0 || input.versions.length > 20) {
      throw new AppError("BAD_REQUEST", "عدد النماذج يجب أن يكون بين 1 و20", 400);
    }
    const quizRows = await this.database.query<{ class_id: string | null; subject_id: string | null; status: string }>(
      "select class_id, subject_id, status::text from quizzes where id = $1",
      [quizId],
    );
    ...
    const normalizedVersions = input.versions.map((version, index) => {
      const key = version.key.trim();
      const label = version.label.trim();
      const lessonIds = [...new Set(version.lessonIds)];
      if (!key || keys.has(key)) throw new AppError("BAD_REQUEST", "مفتاح كل نموذج يجب أن يكون فريدًا", 400);
      keys.add(key);
      ...
      return { ...version, key, label, lessonIds, position: index };
    });
    const allLessonIds = [...new Set(normalizedVersions.flatMap((version) => version.lessonIds))];
    const sourceMap = await this.lessonSources(allLessonIds);
    const units = normalizedVersions.map((version) => {
  ...
  async enqueueQuestionRegeneration(
    ...
    itemId: string,
    input: { clientRequestId: string; subjectDomain: AiSubjectDomain; priority?: number },
  ): Promise<AuthoringPlanResult> {
    const rows = await this.database.query<PublishedQuestionRow>(
      `select i.id as item_id, r.id as revision_id, r.prompt, r.type, r.difficulty,
              i.class_id, i.subject_id,
    ...
    if (!question) throw new AppError("CONFLICT", "إعادة التوليد تتطلب سؤالًا منشورًا وغير مؤرشف", 409);
    const sourceRows = await this.database.query<{ media_asset_id: string; page_number: number; input_checksum_sha256: string; ocr_extraction_id: string | null; content_source_asset_id: string | null; normalized_text: string | null; raw_text: string | null; review_status: "not_required" | "approved" | null }>(
      `select s.media_asset_id, s.page_number, s.input_checksum_sha256, s.ocr_extraction_id,
              s.content_source_asset_id, e.normalized_text, e.raw_text,
              case when e.review_status in ('not_required','approved') then e.review_status::text end as review_status
       from question_bank_revision_sources s
       left join ocr_extractions e on e.id = s.ocr_extraction_id
       where s.revision_id = $1 order by s.position`,
      [question.revision_id],
    );
    if (sourceRows.length === 0) throw new AppError("CONFLICT", "السؤال المنشور لا يملك مصادر موثقة", 409);
    const sourceChunks: AiSourceChunk[] = sourceRows.map((source) => {
    ...
    const request = aiGenerationRequestSchema.parse({
      ...baseRequest("regenerate_question", input.subjectDomain, sourceChunks),
    ...
    return planView(
      await this.createPlan(
        ...
        { kind: "question_regeneration", itemId, publishedRevisionId: question.revision_id },
      ),
    );
  }
  ...
  async applyLessonOutput(actorProfileId: string, outputId: string): Promise<LessonApplyResult> {
    ...
    const lessons = await this.lessonIdentities([lessonId]);
      ...
      const result = await this.database.transaction(async (tx) => {
        const prior = await tx.query<{ id: string }>(
          `select id::text from curriculum_events
           where resource_type = 'lesson' and resource_key = $1 and event_type = 'ai_summary_applied'
             and metadata->>'outputId' = $2 and metadata->>'reviewRevision' = $3
           limit 1`,
          [lessonId, outputId, String(review.revision)],
        );
        if (prior[0]) return { replayed: true };
        const updated = await tx.query<{ id: string }>(
          `update lessons set summary = $2, content_revision = content_revision + 1 where id = $1 returning id`,
          [lessonId, summary],
        );
    ...
    return { lessonId, summaryApplied, summaryReplayed, questionBankItemIds, questionImportReplayed };
  }
  ...
  async archiveQuestion(actorProfileId: string, itemId: string): Promise<{ replayed: boolean }> {
    return this.database.transaction(async (tx) => {
      ...
      const review = await tx.query<{ id: string }>(
        "select id from question_bank_revisions where item_id = $1 and status = 'review' limit 1",
        [itemId],
      );
      if (review[0]) throw new AppError("CONFLICT", "أعد السؤال من المراجعة قبل أرشفته", 409);
      await tx.query("update question_bank_items set archived_at = now() where id = $1", [itemId]);
      await tx.query("update question_bank_revisions set status = 'archived' where item_id = $1 and status <> 'archived'", [itemId]);
      await tx.query(
  ...
  async archiveQuiz(actorProfileId: string, quizId: string): Promise<{ replayed: boolean }> {
    return this.database.transaction(async (tx) => {
      const rows = await tx.query<{ status: string }>("select status::text from quizzes where id = $1 for update", [quizId]);
      const quiz = rows[0];
      if (!quiz) throw new AppError("NOT_FOUND", "الاختبار غير موجود", 404);
  ...
  private async createPlan(
    ...
    for (const unit of units) {
      const unitDefinition = getPromptDefinition(unit.request.mode);
      if (unitDefinition.key !== definition.key || unitDefinition.version !== definition.version) {
    ...
    return this.database.transaction(async (tx) => {
      const result = await this.repository.createPlan(tx, {
        ...
        planFingerprint,
        maxAttempts: 3,
        units: units.map((unit, position) => ({ unitKey: unit.unitKey, position, request: unit.request })),
      });
      if (!result.replayed) {
        await tx.query(
          "update ai_jobs set input_manifest = input_manifest || jsonb_build_object('authoring', $2::jsonb) where id = $1",
  ...
  private async approvedOutput(outputId: string) {
    const contexts = await this.database.query<OutputContextRow>(
      `select o.id as output_id, u.unit_key, u.input_payload, j.prompt_key, j.prompt_version
       from ai_outputs o
    ...
    const reviews = await this.database.query<ReviewRow>(
      `select revision, action, reviewed_output
       from ai_output_review_events where ai_output_id = $1 order by revision desc limit 1`,
      [outputId],
    );
    ...
    if (!parsed.success) throw new AppError("INTERNAL_ERROR", "طلب AI المخزن لا يطابق العقد", 500);
    const output = validateAdminApprovalOutput(parsed.data, review.reviewed_output);
    return { context, review, request: parsed.data, output };
  }

  private async lessonIdentities(lessonIds: readonly string[]): Promise<LessonIdentity[]> {
    if (lessonIds.length === 0) return [];
    return this.database.query<LessonIdentity>(
      "select id, class_id, subject_id, title from lessons where id = any($1::uuid[]) and status <> 'archived'",
      [lessonIds],
    );
  }
  ...
  private async lessonSources(lessonIds: readonly string[]): Promise<Map<string, AiSourceChunk[]>> {
    const rows = await this.database.query<SourceRow>(
      `select l.id as lesson_id, l.class_id, l.subject_id, l.title as lesson_title,
              ...
              o.id as ocr_extraction_id, o.normalized_text, o.raw_text,
              case when o.review_status in ('not_required','approved') then o.review_status::text end as review_status
       from lessons l
       join lesson_assets la on la.lesson_id = l.id and la.publication_status = 'published'
    ...
    for (const row of rows) {
      const text = row.normalized_text?.trim() || row.raw_text?.trim() || null;
      const approvedOcr = Boolean(text && row.ocr_extraction_id && row.review_status);
      const chunks = map.get(row.lesson_id) ?? [];
      chunks.push({
        mediaAssetId: row.media_asset_id,
        pageNumber: row.page_number,
        inputChecksumSha256: row.input_checksum_sha256,
        inputKind: approvedOcr ? "approved_ocr" : "vision_fallback",
        ocrExtractionId: approvedOcr ? row.ocr_extraction_id : null,
        approvedText: approvedOcr ? text : null,
        ocrReviewStatus: approvedOcr ? row.review_status : null,
        contentSourceAssetId: row.content_source_asset_id,
      } as AiSourceChunk);
      if (chunks.length > 64) throw new AppError("CONFLICT", "مصادر الدرس تتجاوز حد 64 صفحة للمهمة الواحدة", 409);
