...
if (!databaseUrl) {
  throw new Error("DATABASE_URL is required for G-D authoring integration tests");
}
...
async function seedLessonMedia(
  ...
  await db.query(
    `insert into ocr_extractions (
       ...
       provider_version, profile_key, status, attempt_count, raw_text,
       normalized_text, review_status, reviewed_by_profile_id, reviewed_at,
  ...
  const assetRows = await db.query<{ id: string }>(
    `insert into lesson_assets (
       lesson_id, kind, position, storage_key, mime_type, byte_size,
       ...
       $1, 'image', 0, $2, 'image/png', 8, $3, 'published', $4,
       now(), $4, now(), 1
     ) returning id`,
    [lessonId, `gd/${marker}-lesson.png`, mediaId, actorId],
  );
  return { mediaId, assetId: assetRows[0]?.id };
}
...
async function approveOutput(
  ...
  await db.query(
    `insert into ai_output_review_events (
       ai_output_id, revision, action, actor_profile_id, reviewed_output
...
test("G-D reuses canonical AI jobs and preserves review/provenance boundaries", async () => {
  ...
  const deviceId = (
    await db.query<{ id: string }>(
      `insert into student_devices (
         profile_id, public_key_spki, public_key_sha256, label
       ) values ($1, $2, $3, 'gd-device')
       returning id`,
  ...
  const lessonRows = await db.query<{ id: string; title: string }>(
    `insert into lessons (class_id, subject_id, slug, title, position)
     values
       ($1, $2, $3, 'درس G-D الأول', 0),
       ($1, $2, $4, 'درس G-D الثاني', 1)
     returning id, title`,
  ...
  try {
    ...
    const summaryUnits = await db.query<{
      id: string;
      unit_key: string;
      input_payload: StoredRequest;
    }>(
      `select id, unit_key, input_payload
       from ai_job_units
       where job_id = $1
       order by position`,
      [summaryJobId],
    ...
    assert.deepEqual(
      summaryUnits.map((unit) => unit.unit_key),
      [`lesson:${lessonOne.id}`, `lesson:${lessonTwo.id}`],
    );
    assert.ok(
      summaryUnits.every(
        (unit) =>
          unit.input_payload.sourceChunks.length === 1 &&
          unit.input_payload.sourceChunks[0]?.inputKind === "approved_ocr",
      ),
    );

    const summaryUnit = summaryUnits[0];
    const summarySource = summaryUnit?.input_payload.sourceChunks[0];
    assert.ok(summaryUnit && summarySource);
    const summaryOutputId = await approveOutput(db, adminId, summaryUnit.id, {
      kind: "summary",
      summary: "ملخص G-D المعتمد",
      sourceEvidence: [
        {
          mediaAssetId: summarySource.mediaAssetId,
          pageNumber: summarySource.pageNumber,
        },
      ],
    });
    const appliedSummary = await app.inject({
      method: "POST",
      url: `/v1/admin/authoring/outputs/${summaryOutputId}/apply-lesson`,
      headers: { cookie: adminCookie, origin },
    });
    assert.equal(appliedSummary.statusCode, 200);
    assert.equal(appliedSummary.json().summaryReplayed, false);
    const summaryReplay = await app.inject({
      method: "POST",
      url: `/v1/admin/authoring/outputs/${summaryOutputId}/apply-lesson`,
      headers: { cookie: adminCookie, origin },
    });
    assert.equal(summaryReplay.json().summaryReplayed, true);
    ...
    const questionUnit = (
      await db.query<{ id: string; input_payload: StoredRequest }>(
    ...
    assert.ok(questionUnit && questionSource);
    const questionOutputId = await approveOutput(db, adminId, questionUnit.id, {
      kind: "question_set",
      questions: [
        {
          ...
          sourceEvidence: [
            {
              mediaAssetId: questionSource.mediaAssetId,
    ...
    const appliedQuestion = await app.inject({
      method: "POST",
      url: `/v1/admin/authoring/outputs/${questionOutputId}/apply-lesson`,
      headers: { cookie: adminCookie, origin },
    });
    assert.equal(appliedQuestion.statusCode, 200);
    assert.equal(appliedQuestion.json().questionBankItemIds.length, 1);
    const questionItemId = appliedQuestion.json().questionBankItemIds[0] as string;
    const questionReplay = await app.inject({
      method: "POST",
      url: `/v1/admin/authoring/outputs/${questionOutputId}/apply-lesson`,
      headers: { cookie: adminCookie, origin },
    });
    assert.equal(questionReplay.json().questionImportReplayed, true);
    ...
    assert.equal(regenerate.statusCode, 202);
    const regenerateRequest = (
      await db.query<{ input_payload: StoredRequest }>(
        "select input_payload from ai_job_units where job_id = $1",
        [regenerate.json().jobId],
      )
    )[0]?.input_payload;
    ...
    assert.equal(quiz.statusCode, 201);
    const quizId = quiz.json().quizId as string;
    ...
    const quizUnits = await db.query<{
      unit_key: string;
      input_payload: StoredRequest;
    }>(
      `select unit_key, input_payload
       from ai_job_units
       where job_id = $1
       order by position`,
      [quizPlan.json().jobId],
    ...
    assert.deepEqual(
      quizUnits.map((unit) => unit.unit_key),
      ["quiz-version:a", "quiz-version:b"],
    );
    ...
    const version = await app.inject({
      ...
      payload: {
        ...
        questions: [
          {
            questionBankItemId: questionItemId,
            questionBankRevisionId: publishedRevisionId,
          },
        ],
      },
    });
    assert.equal(version.statusCode, 201);
    const versionId = version.json().versionId as string;
    assert.equal(
      (
        await app.inject({
          method: "POST",
    ...
    const quizArchiveCount = await db.query<{ count: string }>(
      `select count(*)::text as count
       from quiz_builder_events
       where quiz_id = $1 and action = 'archive'`,
      [quizId],
    );
    assert.equal(Number(quizArchiveCount[0]?.count), 1);
    ...
    assert.equal(archiveReplay.json().replayed, true);
