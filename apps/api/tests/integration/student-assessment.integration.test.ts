...
test(
  "Stage15 Student assessment runtime is entitlement-safe, answer-safe, resumable and deterministic",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      ...
      assert.equal(practiceStart.questions.every((question) => question.feedback === null), true);
      ...
      const choiceFeedback = practiceChoiceAnswered.questions.find((question) => question.id === practiceChoice.id);
      ...
      const directFeedback = practiceDirectAnswered.questions.find((question) => question.id === practiceDirect.id);
      ...
      assert.equal(
        assessmentBody(testCorrectedResponse).questions.find((question) => question.id === testChoice.id)?.feedback,
        null,
      );
      ...
      assert.equal(
        assessmentBody(testDirectCorrectedResponse).questions.find((question) => question.id === testDirect.id)?.feedback,
        null,
      );
      ...
      assert.equal(finalizedTest.questions.every((question) => question.feedback !== null), true);
      ...
      if (profileIds.length > 0) {
        await db.query("delete from quiz_attempts where profile_id = any($1::uuid[])", [profileIds]).catch(() => undefined);
      ...
      if (bankItemIds.length > 0) {
        await db.query("delete from question_bank_items where id = any($1::uuid[])", [bankItemIds]).catch(() => undefined);
      }
      if (profileIds.length > 0) {
