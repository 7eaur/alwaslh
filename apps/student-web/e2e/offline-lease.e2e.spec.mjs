  ...
  return page.evaluate(() =>
    new Promise((resolveValue, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains("leases")) db.createObjectStore("leases", { keyPath: "scopeKey" });
      };
      ...
      request.onsuccess = () => {
        ...
        read.onsuccess = () => { db.close(); resolveValue(read.result); };
      };
    }),
  );
}
...
async function putOfflineLease(page, record) {
  await page.evaluate(
    (value) => new Promise((resolveValue, reject) => {
      const request = indexedDB.open("alwaslh-student-offline");
      request.onupgradeneeded = () => {
        const db = request.result;
        if (!db.objectStoreNames.contains("leases")) db.createObjectStore("leases", { keyPath: "scopeKey" });
      ...
      request.onsuccess = () => {
        ...
        transaction.oncomplete = () => { db.close(); resolveValue(); };
      };
    }),
    record,
  );
...
function cloneLeaseRecord(record, profileId, deviceId) {
  return {
    ...
    profileId,
    deviceId,
    lease: { ...record.lease, profileId, deviceId },
  };
}
...
function expectSameOrRefreshedLeaseRecord(actual, baseline) {
  ...
  expect(actual.lease.grants.map(({ entitlementId, scope, classId }) => ({ entitlementId, scope, classId }))).toEqual(
...
async function loginStudent(page, code, password, expectedPurpose) {
  ...
  const [startResponse, completeResponse, leaseResponse] = await Promise.all([startPromise, completePromise, leasePromise]);
...
test("offline lease persists and cleanup stays scoped across logout and device rebind", async ({ page, context }) => {
  ...
  expect(currentRecord.lease.grants.map(({ entitlementId, scope, classId }) => ({ entitlementId, scope, classId }))).toEqual(
    lease.grants.map(({ entitlementId, scope, classId }) => ({ entitlementId, scope, classId })),
  );
