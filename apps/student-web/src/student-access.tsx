...
const learningDestinations: StudentDestinationDefinition[] = [
  ...
  },
  {
    ...
    description: "احفظ الدروس التي تحتاجها لتكون جاهزة عندما ينقطع الإنترنت.",
    icon: "downloads",
  },
];
...
function StudentShellNavigation({ destination, online, focused = false }: { destination: StudentDestination; online: boolean; focused?: boolean }) {
  if (focused) {
    return <div className="student-shell-toolbar student-shell-toolbar--focused"><StudentNetworkState online={online} /></div>;
  }
  ...
  return (
    <>
      <div className="student-shell-toolbar">
        ...
        <Link className={`student-account-entry ${destination === "account" ? "is-active" : ""}`} to="/app/account" aria-current={destination === "account" ? "page" : undefined}>
          <DestinationIcon kind="account" /><span>حسابي</span>
        </Link>
      </div>
      <nav className="student-adaptive-nav" aria-label="التنقل الرئيسي للطالب">
        ...
        <div className="student-adaptive-nav__links">
          {learningDestinations.map((item) => (
            <Link key={item.key} className={`student-nav-link ${destination === item.key ? "is-active" : ""}`} to={item.href} aria-current={destination === item.key ? "page" : undefined}>
              <DestinationIcon kind={item.icon} /><span>{item.label}</span>
            </Link>
          ))}
        </div>
        <Link className={`student-nav-link student-nav-link--account ${destination === "account" ? "is-active" : ""}`} to="/app/account" aria-current={destination === "account" ? "page" : undefined}>
          <DestinationIcon kind="account" /><span>الحساب</span>
        </Link>
      </nav>
      <nav className="student-bottom-nav" aria-label="التنقل الرئيسي للطالب على الهاتف">
        {learningDestinations.map((item) => (
          <Link key={item.key} className={`student-bottom-nav__item ${destination === item.key ? "is-active" : ""}`} to={item.href} aria-current={destination === item.key ? "page" : undefined}>
            <DestinationIcon kind={item.icon} /><span>{item.shortLabel}</span>
          </Link>
        ))}
      </nav>
    </>
      ...
      <div className="student-home-grid" aria-label="وجهات التعلم">
        {learningDestinations.filter((item) => item.key !== "home").map((item) => (
          <Link className="student-home-card" key={item.key} to={item.href}>
            ...
            <span className="student-home-card__copy"><strong>{item.label}</strong><small>{item.description}</small></span>
    ...
    <div className={`student-shell student-destination--${destination}${focusedReader ? " is-focused-reader" : ""}`} data-student-destination={destination} data-reader-focused={focusedReader || undefined}>
      ...
      <div className="student-destination">
        ...
        {destination === "learn" ? (
          <>
            ...
            {atLearnRoot && activatedAccess ? (
              <div className="form-alert is-success student-route-notice" role="status"><strong>تم تفعيل الصف. أصبح محتواه متاحًا في التعلم.</strong></div>
            ) : null}
        ...
        {destination === "practice" ? (
          <><DestinationHeading destination="practice" /><StudentAssessmentSection online={online} refreshKey={curriculumRefreshKey} onSessionExpired={onSessionExpired} /></>
        ) : null}

        {destination === "downloads" ? (
          <StudentOfflineDownloadsSection profileId={profileId} online={online} refreshKey={curriculumRefreshKey} onSessionExpired={onSessionExpired} />
        ) : null}
        ...
        {destination === "account" ? (
          <StudentAccountExperience
            ...
            onAccessChanged={() => setCurriculumRefreshKey((current) => current + 1)}
          />
        ) : null}
      </div>
    </div>
