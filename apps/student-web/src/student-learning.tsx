  ...
  if (state.status === "loading") {
    return (
      <div className="learning-skeleton" role="status" aria-live="polite" aria-busy="true">
        <span className="sr-only">جاري تحميل موادك ودروسك</span>
        <span />
        <span />
        <span />
      </div>
  ...
  if (state.status === "offline") {
    return (
      <div className="learning-state" role="status">
        ...
        <Link className="secondary-button learning-state__action" to="/app/downloads">
          فتح التنزيلات
        </Link>
      </div>
    );
  ...
  if (catalog.classes.length === 0) {
    return (
      <div className="empty-state">
        ...
        <Link className="secondary-button learning-empty-action" to="/app/account">
          فتح الحساب
        </Link>
      </div>
    );
      ...
      {catalog.classes.map((classRecord) => (
        <section className="learn-class-group" key={classRecord.id} aria-labelledby={`learn-class-${classRecord.id}`}>
          <header className="learn-class-heading">
            <div>
              <p className="eyebrow">الصف</p>
              <h2 id={`learn-class-${classRecord.id}`}>{classRecord.name}</h2>
              {classRecord.description ? <p>{classRecord.description}</p> : null}
            </div>
            <span>{classRecord.subjects.length} مادة</span>
          </header>

          {classRecord.subjects.length === 0 ? (
            <p className="learn-inline-empty">لا توجد مواد متاحة في هذا الصف الآن.</p>
          ) : (
            <ul className="subject-list">
              {classRecord.subjects.map((subject) => (
                <li key={subject.id}>
                  <Link className="subject-list__link" to={studentSubjectHref(subject.id)}>
                    <span className="subject-list__copy">
                      <strong>{subject.name}</strong>
                      <small>{subject.description ?? "افتح المادة لعرض وحداتها ودروسها."}</small>
                    </span>
                    <span className="subject-list__meta">{lessonCount(subject)} درس</span>
                    <span className="subject-list__arrow" aria-hidden="true">←</span>
                  </Link>
                </li>
              ))}
            </ul>
  ...
  return (
    <Link className="lesson-link" data-lesson-id={lesson.id} to={studentLessonHref(lesson.id)}>
      ...
      <span className="lesson-copy">
        ...
        <span className={`lesson-summary ${lesson.summary ? "" : "lesson-muted"}`}>
          {lesson.summary ?? "افتح الدرس لبدء القراءة."}
        </span>
      </span>
      ...
      {subject.unsectionedLessons.length > 0 ? (
        <div className="lesson-group">
          {subject.unsectionedLessons.map((lesson) => {
            ...
            index += 1;
            return <LessonLink key={lesson.id} lesson={lesson} index={lessonIndex} />;
          })}
        </div>
      ) : null}
      ...
      {subject.sections.map((section) => (
        <section className="curriculum-section-group" key={section.id} aria-labelledby={`section-${section.id}`}>
          ...
          {section.lessons.length === 0 ? (
            <p className="learn-inline-empty">لا توجد دروس في هذه الوحدة الآن.</p>
          ) : (
            <div className="lesson-group">
...
function SubjectPage({ catalog, subjectId }: { catalog: StudentCurriculumCatalog; subjectId: string }) {
  ...
  return (
    <article className="subject-page" aria-labelledby={`subject-${subject.id}`}>
      <nav className="learning-breadcrumb" aria-label="مسار التعلّم">
        <Link to="/app/learn">التعلّم</Link>
        <span aria-hidden="true">/</span>
        <span>{classRecord.name}</span>
      </nav>

      <header className="subject-page__header">
        <p className="eyebrow">{classRecord.name}</p>
        <h1 id={`subject-${subject.id}`}>{subject.name}</h1>
        {subject.description ? <p>{subject.description}</p> : null}
        <span className="subject-page__count">{lessonCount(subject)} درس</span>
      </header>

      {lessonCount(subject) === 0 ? (
        <div className="empty-state">
          <strong>لا توجد دروس متاحة في هذه المادة الآن</strong>
          <p>ارجع إلى موادك واختر مادة أخرى.</p>
        </div>
      ) : (
...
export function StudentLearningExperience({
  online,
  refreshKey,
  onSessionExpired,
}: {
  ...
  async function loadCurriculum() {
    ...
    try {
      setState({ status: "ready", catalog: await listStudentCurriculum() });
    } catch (error) {
  ...
  if (state.status !== "ready") {
    return <LearningState state={state} online={online} onRetry={() => void loadCurriculum()} />;
  }
