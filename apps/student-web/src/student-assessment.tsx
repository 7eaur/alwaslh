...
function attemptHref(sessionId: string): string {
  ...
  if (state.status === "offline") {
    return (
      <div className="practice-state" role="status">
        ...
        <p>عندما يعود الاتصال ستتمكن من فتح محاولاتك وبدء تدريب أو اختبار جديد.</p>
      </div>
    );
  }
  ...
  if (state.status === "error") {
    return (
      <div className="practice-state practice-state--error" role="alert">
        ...
        <p>{state.message}</p>
        <button className="secondary-button" type="button" onClick={onRetry} disabled={!online}>
          إعادة المحاولة
        </button>
      </div>
    );
  ...
  return (
    <div className="practice-library">
      <header className="practice-page-heading">
        ...
        <button className="text-button" type="button" onClick={onRetry} disabled={!online || state.status === "loading"}>
          تحديث
        </button>
      </header>

      {!online ? (
        <div className="form-alert is-warning practice-route-notice" role="status">
          أنت غير متصل الآن. يلزم اتصال لبدء محاولة أو استئنافها.
        </div>
      ) : null}
      {state.status === "error" ? (
        <div className="form-alert is-danger practice-route-notice" role="alert">
          {state.message}
        </div>
      ) : null}
      ...
      {state.quizzes.length === 0 ? (
        <div className="empty-state practice-empty">
          <strong>لا توجد تدريبات متاحة لك الآن</strong>
          <p>ستظهر هنا التدريبات والاختبارات المرتبطة بموادك عندما تصبح متاحة لحسابك.</p>
        </div>
      ) : (
        <>
          ...
          {visibleQuizzes.length === 0 ? (
            <div className="empty-state practice-empty">
              <strong>لا توجد نتائج بهذا الاختيار</strong>
              <p>جرّب صفًا أو مادة أخرى.</p>
            </div>
          ) : (
                ...
                <li key={quiz.id} data-quiz-id={quiz.id}>
                  <Link className="practice-list__link" to={quizHref(quiz.id)}>
                    <span className="practice-list__content">
                      <span className="practice-list__context">
                        {quiz.className} · {quiz.subjectName}
                      </span>
                      <strong>{quiz.title}</strong>
                      <small>{quiz.description ?? "افتح التفاصيل واختر تدريبًا أو اختبارًا."}</small>
                    </span>
                    <span className="practice-list__meta">
                      <span>{quiz.versions.length > 1 ? `${quiz.versions.length} مجموعات أسئلة` : "مجموعة أسئلة واحدة"}</span>
      ...
      {state.attempts.length > 0 ? (
        <section className="practice-history" aria-labelledby="practice-history-title">
          <header>
            <div>
              <p className="eyebrow">سجل التعلم</p>
              <h2 id="practice-history-title">آخر المحاولات</h2>
            </div>
              ...
              <li key={attempt.id}>
                <Link to={attemptHref(attempt.sessionId)}>
                  <span>
                    ...
                    <small>
                      {modeLabel(attempt.mode)} · {attemptDate(attempt.completedAt)}
                    </small>
                  </span>
...
function QuizDetail({
  ...
  return (
    <article className="quiz-detail" data-quiz-id={quiz.id} aria-labelledby="quiz-detail-title">
      <nav className="practice-breadcrumb" aria-label="مسار التدريب">
        <Link to="/app/practice">التدريب</Link>
        <span aria-hidden="true">/</span>
        <span>{quiz.subjectName}</span>
      </nav>

      <header className="quiz-detail__hero">
        <p className="eyebrow">{quiz.className} · {quiz.subjectName}</p>
        <h1 id="quiz-detail-title">{quiz.title}</h1>
        <p>{quiz.description ?? "اختر الطريقة المناسبة لك وابدأ عندما تكون جاهزًا."}</p>
      </header>

      {error ? <div className="form-alert is-danger quiz-detail__notice" role="alert">{error}</div> : null}
      {!online ? (
        <div className="form-alert is-warning quiz-detail__notice" role="status">
          يلزم اتصال لبدء التدريب أو الاختبار.
        </div>
      ) : null}
      ...
      {quiz.versions.length > 1 ? (
        <section className="quiz-choice-section" aria-labelledby="question-set-title">
          ...
          <div className="field-group quiz-question-set-field">
            <label htmlFor={`assessment-version-${quiz.id}`}>مجموعة الأسئلة</label>
            <select
            ...
            <p className="field-hint">
              {selectedVersion ? `المجموعة المختارة: ${selectedVersion.label}` : "سيتم اختيار مجموعة متاحة عند البدء."}
            </p>
          </div>
        </section>
      ) : (
        <p className="quiz-single-set">مجموعة أسئلة واحدة جاهزة للبدء.</p>
      )}

      <section className="quiz-mode-choice" aria-labelledby="quiz-mode-title">
        <header>
          <p className="eyebrow">اختر طريقتك</p>
          <h2 id="quiz-mode-title">كيف تريد الحل؟</h2>
        </header>
        <div className="quiz-mode-options">
          <button
            type="button"
            className="quiz-mode-option"
          ...
          >
            ...
            <span>
              <strong>تدريب</strong>
              <small>اعرف نتيجة كل إجابة مباشرة وتعلّم من التصحيح أثناء الحل.</small>
            </span>
          ...
          >
            ...
            <span>
              ...
              <small>أجب عن جميع الأسئلة أولًا، ثم شاهد نتيجتك والتصحيح في النهاية.</small>
            </span>
...
function QuestionFeedback({ question }: { question: StudentAssessmentQuestion }) {
  ...
  return (
    <div
      className={`attempt-feedback ${question.feedback.correct ? "is-correct" : "is-incorrect"}`}
      role="status"
      aria-live="polite"
    >
      ...
      {!question.feedback.correct && question.feedback.correctAnswerText ? (
        <p>الإجابة الصحيحة: {question.feedback.correctAnswerText}</p>
      ) : null}
  ...
  if (!question) {
    return (
      <div className="practice-state">
        <strong>لا توجد أسئلة في هذه المحاولة</strong>
        <p>ارجع إلى التدريب واختر نشاطًا آخر.</p>
    ...
    return (
      <div className="attempt-review-shell">
        ...
        <section className="attempt-review" aria-labelledby="attempt-review-title">
          <header>
            <p className="eyebrow">مراجعة الإجابات</p>
            <h2 id="attempt-review-title">راجع ما أجبت عنه</h2>
          </header>
          <nav className="attempt-question-nav" aria-label="أسئلة المحاولة">
            {assessment.questions.map((candidate, index) => (
              <button
                ...
                onClick={() => setQuestionId(candidate.id)}
              >
                {index + 1}
              </button>
            ))}
  ...
  return (
    <section className="attempt-shell" aria-labelledby="attempt-title">
      <header className="attempt-topbar">
        <Link className="attempt-exit" to="/app/practice" aria-label="الخروج من المحاولة والعودة إلى التدريب">
          <span aria-hidden="true">→</span>
          <span>التدريب</span>
        </Link>
      ...
      <div className="attempt-progress" aria-label={`أجبت عن ${assessment.progress.answeredCount} من ${assessment.progress.questionCount} سؤال`}>
        <span style={{ width: `${progressPercent}%` }} />
      </div>

      {!online ? (
        <div className="attempt-connection-state" role="status">
          انقطع الاتصال. يمكنك قراءة السؤال، لكن الحفظ والإنهاء متوقفان حتى يعود الاتصال.
        </div>
      ) : null}
      {assessment.session.mode === "test" ? (
        <p className="attempt-mode-note">في الاختبار يظهر التصحيح بعد إنهاء جميع الأسئلة.</p>
      ) : null}
      ...
      <article className="attempt-question" data-question-id={question.id}>
        <header>
          <p className="attempt-question__meta">
        ...
        {question.type === "direct" ? (
          <div className="field-group attempt-direct-field">
            ...
            <textarea
              ...
              className="text-input attempt-direct-answer"
              value={directAnswer}
              onChange={(event) => setDirectAnswer(event.target.value)}
              disabled={Boolean(question.feedback) || busy || !online}
              maxLength={10_000}
              rows={5}
            />
          </div>
        ...
        {!question.feedback ? (
          <button
            className="primary-button attempt-primary-action"
            type="button"
            onClick={() => void saveAnswer()}
            disabled={busy || !online}
          >
            {busy ? "جاري الحفظ" : assessment.session.mode === "practice" ? "تحقق من إجابتي" : "حفظ الإجابة"}
          </button>
        ) : null}
      ...
      <footer className="attempt-finish-bar">
        <div>
          <strong>{allAnswered ? "أكملت جميع الأسئلة" : `بقي ${assessment.progress.questionCount - assessment.progress.answeredCount} سؤال`}</strong>
          <small>{allAnswered ? "يمكنك إنهاء المحاولة ومشاهدة النتيجة." : "أجب عن كل الأسئلة قبل الإنهاء."}</small>
        </div>
        <button
...
function AttemptPage({
  ...
  async function loadAttempt() {
    ...
    try {
      const assessment = await getStudentAssessmentSession(sessionId);
      ...
      if (error instanceof ApiRequestError && error.code === "NOT_FOUND") {
        setState({ status: "error", message: error.message, unavailable: true });
        return;
      }
  ...
  if (state.status === "loading") {
    return (
      <div className="attempt-loading" role="status" aria-live="polite" aria-busy="true">
        <span className="spinner" aria-hidden="true" />
        <strong>نفتح محاولتك</strong>
      </div>
    );
  }
  ...
  if (state.status === "offline") {
    return (
      <div className="practice-state">
        <strong>نحتاج اتصالًا لفتح المحاولة</strong>
        <p>أعد الاتصال ثم حاول مرة أخرى.</p>
        <button className="secondary-button" type="button" onClick={() => void loadAttempt()} disabled={!online}>
          إعادة المحاولة
        </button>
      </div>
    );
  ...
  if (state.status === "error") {
    return (
      <div className="practice-state practice-state--error" role="alert">
        <strong>{state.unavailable ? "هذه المحاولة لم تعد متاحة" : "تعذر فتح المحاولة"}</strong>
        <p>{state.message}</p>
        <Link className="secondary-button" to="/app/practice">العودة إلى التدريب</Link>
      </div>
    );
  ...
  return (
    <AssessmentWorkspace
      ...
      onChange={(assessment) => setState({ status: "ready", assessment })}
      onUnavailable={(message) => setState({ status: "error", message, unavailable: true })}
    />
  );
}

export function StudentAssessmentExperience({
  online,
  refreshKey,
  onSessionExpired,
}: {
  ...
  useEffect(() => {
    if (route.kind !== "attempt") void loadCatalog();
  }, [online, refreshKey, route.kind]);
  ...
  if (route.kind === "attempt") {
    return <AttemptPage sessionId={route.sessionId} online={online} onSessionExpired={onSessionExpired} />;
  }
  ...
  if (!quiz) {
    return (
      <div className="practice-state practice-state--permission" role="status">
        ...
        <p>قد تكون صلاحية الوصول تغيّرت أو أن الرابط لم يعد صالحًا.</p>
        <Link className="secondary-button" to="/app/practice">العودة إلى التدريب</Link>
      </div>
    );
  }
