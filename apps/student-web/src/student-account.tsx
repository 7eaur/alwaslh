...
export function StudentAccountExperience({
  ...
  return (
    <main className="student-account-experience" aria-labelledby="student-account-title">
      <header className="student-account-hero">
        <div>
          <p className="eyebrow">حسابي</p>
          <h1 id="student-account-title">{displayName ? `مرحبًا ${displayName}` : "الحساب والوصول"}</h1>
          <p>راجع الصفوف المتاحة لحسابك، وأضف رمز صف جديدًا عندما تحتاج إليه.</p>
        </div>
        <button className="secondary-button student-logout-button" type="button" onClick={onLogout} disabled={logoutBusy}>
      ...
      <section className="student-account-section" aria-labelledby="active-access-title">
        <div className="student-account-section__heading">
          <div>
            <p className="eyebrow">المحتوى المتاح</p>
            <h2 id="active-access-title">وصولك الحالي</h2>
          </div>
          <button className="text-button" type="button" onClick={() => void loadAccess()} disabled={!online || state.status === "loading"}>
            تحديث
          </button>
        </div>
        ...
        {state.status === "loading" ? (
          <div className="student-b05-skeleton" role="status" aria-live="polite" aria-busy="true">
            <span className="sr-only">جاري تحميل وصولك الحالي</span><span /><span />
          </div>
        ) : state.status === "offline" ? (
          <div className="student-b05-state is-warning" role="status">
            <strong>أنت غير متصل الآن</strong>
            <p>يمكنك متابعة ما سبق تنزيله. اتصل بالإنترنت لتحديث الوصول أو إضافة رمز صف.</p>
          </div>
        ) : state.status === "error" ? (
          <div className="student-b05-state is-error" role="alert">
            <strong>تعذر تحميل الوصول</strong>
            <p>{state.message}</p>
            <button className="secondary-button" type="button" onClick={() => void loadAccess()} disabled={!online}>إعادة المحاولة</button>
          </div>
        ) : entitlements.length === 0 ? (
          <div className="student-b05-state is-empty">
            <strong>لا توجد صفوف مفعّلة لحسابك الآن</strong>
            <p>إذا حصلت على رمز صف، أضفه من القسم التالي ليظهر محتواه في التعلم.</p>
          </div>
        ) : (
          <ul className="student-access-list" aria-label="الوصول الحالي">
            {entitlements.map((entitlement) => (
              <li key={entitlement.id}>
                ...
                <span>
                  <strong>{entitlement.scope === "all_content" ? "كل المحتوى المتاح" : "صف مفعّل"}</strong>
      ...
      <section className="student-account-section student-account-activation" aria-labelledby="class-code-title">
        <div className="student-account-section__heading">
          <div>
            <p className="eyebrow">إضافة صف</p>
            <h2 id="class-code-title">لديك رمز صف جديد؟</h2>
            <p>اكتب الرمز المكوّن من 7 أرقام كما استلمته.</p>
          </div>
        </div>

        {hasFullAccess ? (
          <div className="student-b05-state is-info" role="status">
            <strong>لديك وصول كامل</strong>
            <p>لا تحتاج إلى إضافة رمز صف آخر الآن.</p>
          </div>
        ) : (
            ...
            <div className="field-group">
              ...
              <div className="student-account-code-row">
                <input
                  ...
                  dir="ltr"
                />
                <button className="primary-button" type="submit" disabled={!isSevenDigitClassCode(normalizedClassCode) || redeemBusy || !online}>
