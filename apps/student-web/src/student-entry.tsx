...
export function initialStudentEntryMode(): StudentEntryMode {
  try {
...
export function markStudentWelcomeSeen(): void {
  try {
    ...
    // Welcome state is helpful UX only; storage denial must not block entry.
  }
}
...
function learnerErrorMessage(error: unknown): string {
  if (error instanceof ApiRequestError) return error.message;
  if (error instanceof Error && error.message === "device_key_missing") {
    return "تعذر التعرف على هذا الجهاز. اطلب من الدعم إعادة ربط الجهاز ثم جرّب تسجيل الدخول مرة أخرى.";
  }
  if (error instanceof Error && error.message === "device_crypto_unavailable") {
    return "تعذر تسجيل الدخول من هذا المتصفح. حدّث المتصفح أو استخدم متصفحًا حديثًا ثم أعد المحاولة.";
  }
  ...
  return (
    <>
      <div className="field-group">
        ...
        <p className="field-hint" id={`${prefix}-password-hint`}>استخدم 8 أحرف على الأقل، واختر كلمة يسهل عليك تذكرها ولا تشاركها مع أحد.</p>
      </div>
      <div className="field-group">
...
function ActivationForm({ online, onAuthenticated, onLogin }: {
  ...
  async function handleVerify(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(learnerErrorMessage(requestError));
    } finally {
      setBusy(false);
    }
  }
  ...
  if (verification) {
    return (
      <form className="student-entry-form" onSubmit={handleComplete} noValidate>
        {error ? <FormAlert tone="danger">{error}</FormAlert> : null}
        <FormAlert tone="success">الرمز صالح. اختر كلمة مرور لحسابك ثم أكمل.</FormAlert>
        ...
        <button className="primary-button full-width" type="submit" disabled={!canComplete || busy || !online}>{busy ? <><Spinner /> جاري إنشاء الحساب</> : "إنشاء الحساب والمتابعة"}</button>
        <button className="text-button full-width" type="button" disabled={busy} onClick={() => resetVerification("")}>استخدام رمز مختلف</button>
      </form>
    );
  }

  return (
    <form className="student-entry-form" onSubmit={handleVerify} noValidate>
      {error ? <FormAlert tone="danger">{error}</FormAlert> : null}
      <div className="field-group">
        <label htmlFor="activation-code">رمز التفعيل</label>
        <input id="activation-code" className="text-input code-input" type="text" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9٠-٩۰-۹]*" maxLength={12} value={normalizedCode} onChange={(event) => resetVerification(event.target.value)} aria-describedby="activation-code-hint" placeholder="000000" dir="ltr" />
      ...
      <button className="primary-button full-width" type="submit" disabled={!canVerify || busy || !online}>{busy ? <><Spinner /> جاري التحقق</> : "متابعة"}</button>
      <button className="text-button full-width" type="button" onClick={onLogin} disabled={busy}>لدي حساب بالفعل</button>
    </form>
  );
}

async function deviceKeyForChallenge(identifier: string, challenge: StudentLoginChallenge): Promise<StoredDeviceKey> {
  if (challenge.purpose === "device_rebind" || challenge.purpose === "password_change_rebind") return rotateDeviceKey(identifier);
...
function LoginForm({ online, notice, onAuthenticated, onRecovery }: {
  ...
  async function finishChallenge(challenge: StudentLoginChallenge, key: StoredDeviceKey) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await finishChallenge(challenge, key);
    } catch (requestError) {
      setError(learnerErrorMessage(requestError));
    } finally {
      setBusy(false);
  ...
  if (pending) {
    return (
      <form className="student-entry-form" onSubmit={handlePasswordChange} noValidate>
        {error ? <FormAlert tone="danger">{error}</FormAlert> : null}
        <FormAlert tone="info">استخدمت كلمة مرور مؤقتة. اختر الآن كلمة مرور جديدة خاصة بك.</FormAlert>
        ...
        <button className="primary-button full-width" type="submit" disabled={newPassword.length < 8 || newPassword !== confirmation || busy || !online}>{busy ? <><Spinner /> جاري الحفظ</> : "حفظ والدخول"}</button>
        <button className="text-button full-width" type="button" disabled={busy} onClick={() => setPending(null)}>العودة</button>
      </form>
    );
  }

  return (
    <form className="student-entry-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="field-group">
        ...
        <p className="field-hint" id="student-identifier-hint">هو رمز التفعيل المكوّن من 6 أرقام الذي أنشأت به حسابك.</p>
      </div>
      <div className="field-group">
      ...
      <button className="primary-button full-width" type="submit" disabled={!valid || busy || !online}>{busy ? <><Spinner /> جاري الدخول</> : "تسجيل الدخول"}</button>
    </form>
  );
}
...
function WelcomePanel({ onStart, onLogin, onHelp }: { onStart: () => void; onLogin: () => void; onHelp: () => void }) {
  return (
    <div className="student-welcome-panel">
      ...
      <h1>تعلّم، تدرّب، وارجع لما تحتاجه بسهولة.</h1>
      <p>الوسيلة الذكية تجمع موادك ودروسك وتدريباتك في تجربة بسيطة ومناسبة للهاتف والكمبيوتر.</p>
      <ul className="student-welcome-points">
        ...
        <li><strong>احتفظ بما تحتاجه</strong><span>احفظ الدروس المتاحة على جهازك لتكون جاهزة عند الحاجة.</span></li>
      </ul>
      <div className="student-entry-actions">
        <button className="primary-button" type="button" onClick={onStart}>ابدأ الآن</button>
...
export function StudentHelpContent({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`student-guidance ${compact ? "is-compact" : ""}`}>
      ...
      <ol className="student-guidance-steps">
        ...
        <li><span>3</span><div><strong>استخدم التدريب والاختبارات</strong><p>التدريب يعطيك ملاحظات أثناء الحل، بينما الاختبار يعرض النتيجة بعد الإنهاء.</p></div></li>
        <li><span>4</span><div><strong>احفظ ما تحتاجه على جهازك</strong><p>من التنزيلات يمكنك حفظ الدروس المتاحة وإدارتها. بعض الاستخدام بدون إنترنت يكتمل ضمن مراحل التطبيق القادمة.</p></div></li>
...
export function StudentSupportContent({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`student-guidance ${compact ? "is-compact" : ""}`}>
      <header><p className="eyebrow">الدعم والتواصل</p><h1>إذا واجهتك مشكلة، ابدأ من هنا</h1><p>لن نطلب منك إرسال كلمة المرور أو أي بيانات سرية للدعم.</p></header>
      <div className="student-support-list">
        <section><strong>نسيت كلمة المرور</strong><p>اطلب كلمة مرور مؤقتة من الجهة التي زودتك برمز التفعيل، ثم استخدمها في شاشة تسجيل الدخول لاختيار كلمة جديدة.</p></section>
        ...
        <section><strong>درس أو اختبار غير ظاهر</strong><p>تحقق من أن وصول الصف ما زال فعالًا، ثم حدّث الصفحة. إذا استمرت المشكلة اذكر اسم الصف والمادة عند التواصل مع الدعم.</p></section>
      </div>
      <div className="student-support-contact" role="note"><strong>قناة التواصل الحالية</strong><p>تواصل مع الإدارة أو الجهة التعليمية التي سلمتك رمز التفعيل. لا توجد في العقود الحالية قناة رقمية موحدة معتمدة يمكن عرضها هنا دون اختلاق بيانات اتصال.</p></div>
    </div>
  );
}

function RecoveryPanel({ onLogin, onSupport }: { onLogin: () => void; onSupport: () => void }) {
  return (
    <div className="student-entry-form">
      <div className="student-b05-state is-info"><strong>استعادة الحساب</strong><p>إذا نسيت كلمة المرور، اطلب كلمة مرور مؤقتة من الجهة التي زودتك برمز التفعيل. بعدها ارجع إلى تسجيل الدخول وسيطلب منك التطبيق اختيار كلمة مرور جديدة.</p></div>
    ...
    <main className="student-entry-page">
      <div className="student-entry-shell">
        ...
        <section className={`student-entry-main ${authMode ? "is-auth" : "is-info"}`}>
          {mode === "welcome" ? <WelcomePanel onStart={() => { markStudentWelcomeSeen(); onMode("activation"); }} onLogin={() => { markStudentWelcomeSeen(); onMode("login"); }} onHelp={() => onMode("help")} /> : null}
          {authMode ? (
            <div className="student-auth-surface">
              ...
              <header className="student-auth-heading">
                <p className="eyebrow">مساحة الطالب</p>
                <h1>{mode === "activation" ? "تفعيل حساب جديد" : mode === "login" ? "تسجيل الدخول" : "استعادة الحساب"}</h1>
                <p>{mode === "activation" ? "أدخل رمز التفعيل ثم اختر كلمة مرور لحسابك." : mode === "login" ? "أدخل معرّف حسابك وكلمة المرور للمتابعة." : "اتبع الخطوات البسيطة لاستعادة الوصول إلى حسابك."}</p>
              </header>
              ...
              {mode === "recovery" ? <RecoveryPanel onLogin={() => onMode("login")} onSupport={() => onMode("support")} /> : null}
            </div>
          ) : null}
        ...
        <footer className="student-entry-footer">
          ...
          <button className="text-button" type="button" onClick={() => onMode("support")}>الدعم والتواصل</button>
        </footer>
      </div>
    </main>
  );
