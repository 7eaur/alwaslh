...
function ActivationForm({ online, onLogin }: ActivationFormProps) {
  const [code, setCode] = useState("");
  const [blocked, setBlocked] = useState(false);
  const normalizedCode = normalizeAccessCode(code).slice(0, 6);
  const valid = isSixDigitAccessCode(normalizedCode);
    ...
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      <div className="field-group">
        ...
        <p className="field-hint" id="activation-code-hint">
          أدخل 6 أرقام. يمكنك استخدام الأرقام العربية أو الإنجليزية.
        </p>
      </div>

      {!online ? <FormAlert tone="warning">يلزم اتصال بالشبكة قبل إرسال رمز التفعيل.</FormAlert> : null}
      {blocked ? (
        <FormAlert tone="info">
          إكمال إنشاء حساب جديد متوقف مؤقتًا حتى تتوفر خدمة التفعيل الآمنة. لم يتم إرسال الرمز أو استهلاكه.
        </FormAlert>
      ) : null}

      <button className="primary-button full-width" type="submit" disabled={!valid || !online}>
        التحقق والمتابعة
      </button>
      <button className="text-button full-width" type="button" onClick={onLogin}>
        لدي حساب بالفعل
      </button>
    </form>
  );
...
function LoginForm({ online, notice, onAuthenticated, onRecovery }: LoginFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      const profile = await loginStudent(identifier.trim(), password);
      if (profile.role !== "student") {
        void logoutStudent().catch(() => undefined);
        setError("هذا الحساب ليس حساب طالب. استخدم بوابة الإدارة المخصصة للحسابات الإدارية.");
        return;
      }
...
function RecoveryForm({ online, onReset, onLogin }: RecoveryFormProps) {
  ...
  return (
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      <FormAlert tone="info">
        إذا كان لديك رمز استرداد صالح يمكنك تعيين كلمة مرور جديدة هنا. إصدار رمز استرداد من داخل تطبيق الطالب غير متاح حاليًا.
      </FormAlert>
      {error ? <FormAlert tone="danger">{error}</FormAlert> : null}
...
function EntryScreen({ mode, online, notice, onModeChange, onAuthenticated, onRecoveryReset }: EntryScreenProps) {
  const headings: Record<EntryMode, { eyebrow: string; title: string; description: string }> = {
    activation: {
      eyebrow: "طالب جديد",
      title: "فعّل وصولك",
      description: "ابدأ برمز الوصول الكامل المكوّن من 6 أرقام. التفعيل يختلف عن تسجيل الدخول لحساب موجود.",
    },
    login: {
      eyebrow: "مرحبًا بعودتك",
      title: "سجّل دخولك",
      description: "استخدم معرّف حسابك وكلمة المرور. سنعيد الجلسة فقط بعد تحقق الخادم منها.",
    },
    recovery: {
      eyebrow: "استرداد آمن",
      ...
      <div className="entry-layout">
        <aside className="entry-intro" aria-label="معلومات الدخول">
          ...
          <ul className="trust-list">
            <li><span aria-hidden="true">01</span><div><strong>تفعيل جديد</strong><small>رمز وصول كامل من 6 أرقام.</small></div></li>
            <li><span aria-hidden="true">02</span><div><strong>عودة للحساب</strong><small>جلسة موثوقة أو معرّف وكلمة مرور.</small></div></li>
        ...
        <section className="auth-card" aria-labelledby="auth-title">
          ...
          {mode === "activation" ? <ActivationForm online={online} onLogin={() => onModeChange("login")} /> : null}
          {mode === "login" ? (
            <LoginForm online={online} notice={notice} onAuthenticated={onAuthenticated} onRecovery={() => onModeChange("recovery")} />
          ) : null}
          {mode === "recovery" ? <RecoveryForm online={online} onReset={onRecoveryReset} onLogin={() => onModeChange("login")} /> : null}
        </section>
      </div>
    </main>
  );
...
function AuthenticatedScreen({ profile, online, onLoggedOut }: AuthenticatedScreenProps) {
    ...
    <main className="account-page">
      ...
      <section className="account-summary" aria-labelledby="account-title">
        <div>
          ...
          <p>هذه شاشة نجاح لمسار الحساب فقط. بقية تجربة الطالب تبقى خارج نطاق دفعة التفعيل الحالية.</p>
        </div>
      ...
      <section className="access-section" aria-labelledby="access-title">
        ...
        {access.status === "loading" ? (
          <div className="inline-state" role="status"><Spinner /> جاري التحقق من الصلاحيات…</div>
        ) : null}
        {access.status === "offline" ? (
          <FormAlert tone="warning">لا نعرض صلاحيات مخزنة على أنها محدثة. أعد الاتصال للتحقق من الوصول.</FormAlert>
        ) : null}
        {access.status === "error" ? (
...
export function App() {
  ...
  const [mode, setMode] = useState<EntryMode>(() => (window.location.pathname.includes("student-login") ? "login" : "activation"));
  ...
  if (phase === "offline" && !profile) return <ConnectionGate kind="offline" onRetry={() => void restoreSession()} />;
  if (phase === "unavailable" && !profile) return <ConnectionGate kind="unavailable" onRetry={() => void restoreSession()} />;
  ...
  return (
    <div className="app-frame">
      ...
      <EntryScreen
        ...
        onAuthenticated={(studentProfile) => {
