...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // تحديث التوقيع المخزن إلى التوقيع الجديد المستقر عند اكتشاف الجهاز
    // (يحدث مثلاً إذا كان التوقيع القديم يعتمد على اتجاه الشاشة)
    ...
    return new Response(
      JSON.stringify({
        success: true,
        code: row.code,
        has_recovery_password: !!row.recovery_password_encrypted,
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err: any) {
    console.error("get-code-by-device error:", err);
