...
5. إذا كانت الصورة أصلًا WebP محسنة ولم ينتج عن إعادة الترميز ملف أصغر، تحفظ الأداة بايتات WebP الأصلية بدل زيادة الحجم أو إضافة جيل ضغط جديد.
6. تحفظ النص، confidence، blocks/polygons، checksums، العناوين، الصف/المادة/الدرس/الصفحة.
7. تنتج package + JSONL + CSV + HTML review يمكن استيرادها لاحقًا إلى «الوسيلة».
8. تدعم resume بالـSHA-256: الصفحة التي لم تتغير لا يعاد ضغطها أو OCR لها.
...
ملف OCR العربي يستخدم افتراضيًا detector/recognizer خفيفين مناسبين للتشغيل المحلي على CPU، ويعطّل مسار oneDNN غير المستقر الذي ظهر في الاختبار الواقعي على PaddlePaddle 3.x.
...
- WebP موجود أصلًا: لا يعاد الاحتفاظ بالنسخة الجديدة إذا كانت أكبر؛ يحفظ المصدر WebP byte-for-byte عندما يكون هو الخيار الأصغر الآمن
- OCR language: `ar`
- التنفيذ: صفحة واحدة في كل مرة لتقليل RAM
- low confidence threshold: `0.80`
- المراجعة: الصفحة توسم `needs_review` إذا كان متوسط الثقة منخفضًا **أو** إذا كانت 10% أو أكثر من blocks تحت threshold
...
**مهم:** `mean_confidence` هو ثقة نموذج OCR وليس نسبة دقة حقيقية للنص. الجداول والمعادلات والرموز قد تحتوي أخطاء محلية حتى لو كان المتوسط مرتفعًا. لقياس الدقة الفعلية استخدم benchmark مع نصوص ground truth؛ التفاصيل في [BENCHMARK.md](BENCHMARK.md).
...
- WebP المصدر الجيد لا يعاد ضغطه إذا لم تحقق إعادة الترميز توفيرًا فعليًا.
